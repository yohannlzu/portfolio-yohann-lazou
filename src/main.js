...
/* Projects — open details panel (shared, keeps cards aligned) */
(() => {
  ...
  const PROJECTS = [
    {
      ...
      desc:
        "Situation d'Apprentissage et d'Évaluation : conception CATIA, simulation + gamme VBend, documents découpe, fabrication presse plieuse.",
      bullets: [
    ...
    },
    {
      ...
      title: "Foil",
      desc:
        "Réalisation d'un foil en surfacique et de son moule sur CATIA : courbes guides, nuage de points, sections/surfaces, paramétrage du congé.",
      bullets: [
  ...
  const render = (p) => {
    ...
    if (mediaEl) {
      mediaEl.innerHTML = p.images
        .map((img) => {
          const isPieceFinie = (img.cap || "").trim().toLowerCase() === "pièce finie";
...
async function initEarth() {
  ...
  if (loadEl) loadEl.textContent = "Chargement…";
...
function animLoop() {
  ...
  const d =
    Math.abs(targetLon - animLon) +
    Math.abs(targetLat - animLat) +
    Math.abs(targetZoom - animZoom);
...
initEarth().catch((err) => {
  console.error(err);
  if (loadEl) loadEl.textContent = "Erreur de chargement";
});